...
class Option():
    ...
    def set_type(self):
        ...
        elif self.type == "Put on put":
            Put_on_put(self,self.E)      
        else: 
            return        
...
class Call_on_call(Option):
    ...
    def C1_value_tree(self):
        ...
        for n in range(0,self.M1+1):
            payoff = max(self.V_tree[self.M-self.M1][n] - self.E1, 0)
...
class Put_on_put(Option):
    #put on call P1 allows holder to buy a call P2 on an underlying asset at time T1 where P2 has expiry T2
    #we deal with the call P2 firstly then work to value P1 from this 
    def __init__(self,asset,E):
    ...
    def print_option_conditions(self):
        print("Option is a Put P1, with expiry {} and strike price {} on an underlying put P2.".format(self.T1,self.E1))
        print("Where P2 has expiry {} and strike price {} on an underlying asset S".format(self.T2,self.E2))
    ...
    def payoff(self,m,n): 
        #gets the payoff for the put P2, this will be used at top/max timestep M in the tree for euro put and at each step for american put
        #print(self.asset_tree[m][n])
        V = max(self.E2-self.asset_tree[m][n],0)
        return V  
...
#This has been fixed although could possibly be done better ???
...
#put_on_put_test = Option(share, "Put on put", 2.5) #last number is the strike price E2 for put P2
